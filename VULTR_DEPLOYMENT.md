...
- 共通: `SSH_HOST`, `SSH_USER`, `SSH_PRIVATE_KEY`（または `SSH_PRIVATE_KEY_B64`）
- アプリ固有: `.env` で使う全変数
  - `CONTRACT_ADDRESS`
  - `POLYGON_RPC_URL`
  - `GAS_WEBHOOK_URL`
...
- トリガーブランチ: `main`
- SSH接続先Secrets: `SSH_HOST`, `SSH_USER`, `SSH_PRIVATE_KEY`（推奨）
- 改行崩れ対策が必要な場合は `SSH_PRIVATE_KEY_B64` も利用可能
...
## 8. トラブルシューティング（`error in libcrypto` エラー）

GitHub Actions で次のエラーが出る場合：

- `Error loading key "(stdin)": error in libcrypto`
- `Error: Command failed: ssh-add -`

これは多くの場合、**秘密鍵の改行コード（CRLF）混入**や、**秘密鍵本文の欠損/余分文字**が原因です。

### 8.1 よくある原因

- Windows でコピーした際に `\r`（CR）が混入
- Secret 貼り付け時に先頭/末尾の行が欠けた
- 公開鍵（`.pub`）を誤って `SSH_PRIVATE_KEY` に登録
- パスフレーズ付き鍵を利用していて、非対話環境で復号できない

### 8.2 正しい登録内容のチェック

`SSH_PRIVATE_KEY` には **秘密鍵**を登録します（公開鍵ではない）。

- Ed25519 の例: `-----BEGIN OPENSSH PRIVATE KEY-----`
- RSA(PEM) の例: `-----BEGIN RSA PRIVATE KEY-----`

### 8.3 ローカルでの事前検証（推奨）

```bash
# 鍵ファイルが読めるか確認
ssh-keygen -y -f ~/.ssh/id_ed25519_github_actions > /dev/null
```

このコマンドが失敗する鍵は GitHub Actions でも失敗します。

### 8.4 本リポジトリ側の改善点

このリポジトリの `deploy.yml` では、鍵投入時に次を実施します。

- `tr -d '\r'` による CR 除去
- `ssh-keygen -yf` で鍵の妥当性チェック
- エラー時に明示メッセージを出して即終了

これにより `libcrypto` 系エラーの切り分けがしやすくなります。



### 7.7 `webfactory/ssh-agent` がログに出る場合

現在の `deploy.yml` は `webfactory/ssh-agent` を使いません。
ログに `Run webfactory/ssh-agent@...` が出る場合は、**古いコミットのworkflow実行**です。

対処:

1. `main` が最新コミットを指しているか確認
2. GitHub Actions の対象実行が最新コミットSHAか確認
3. 必要なら `Re-run jobs` ではなく、最新 `main` に対して新規 push して再実行


### 8.5 Base64 Secret を使う方法（改行崩れ回避）

`SSH_PRIVATE_KEY` の改行管理が難しい場合、`SSH_PRIVATE_KEY_B64` を使えます。

ローカルで作成:

```bash
base64 -w 0 ~/.ssh/id_ed25519_github_actions > /tmp/id_ed25519_github_actions.b64
cat /tmp/id_ed25519_github_actions.b64
```

出力値を GitHub Secret `SSH_PRIVATE_KEY_B64` に登録してください。
ワークフロー側で自動デコードされます。
